...
export async function getLinkPreview(
  ...
  // Check if text is empty or not and if it's a string
  if (!text || typeof text !== `string`) {
    throw new Error(`did not receive a valid url or text`);
  }
  ...
  if (!detectedUrl) {
    throw new Error(`did not receive a valid a url or text`);
  }

  if (options?.followRedirects === `manual` && !options?.handleRedirects) {
    throw new Error(
      `followRedirects is set to manual, but no handleRedirects function was provided`
    );
  }
